...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    auto buft_for_tensor = [&](ggml_tensor * t_meta) -> ggml_backend_buffer_type_t {
        ...
        // NOTE: We skip this override for GPU host buffers (ROCm_Host/CUDA_Host)
        // because the async copy path now handles host→device copies directly,
        // avoiding the slow sync fallback that CPU buffers require.
        // The ROCm_Host buffer uses `cudaMemcpyAsync(HostToDevice)` on the dest
        // stream (see ggml_backend_cuda_cpy_tensor_async).
