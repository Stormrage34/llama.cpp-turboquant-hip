...
// Custom KV cache quant types (turbo, planar, iso, rq)
...
void quantize_row_rq_mse_ref(const float * GGML_RESTRICT x, block_rq_mse * GGML_RESTRICT y, int64_t k);
...
void dequantize_row_rq_mse(const block_rq_mse * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k);
