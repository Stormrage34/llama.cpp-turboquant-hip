...
static void turbo_cpu_fwht(float * x, int group_size) {
    ...
    // normalize and signs2
    for (int i = 0; i < group_size; i++) x[i] = x[i] * inv_sqrt * s2[i];
}
...
void quantize_row_turbo4_0_ref(const float * GGML_RESTRICT x, block_turbo4_0 * GGML_RESTRICT y, int64_t k) {
    ...
    for (int block = 0; block < nb; block++) {
        ...
        static const float CENTROIDS_4BIT[16] = {
            -0.235859f, -0.179174f, -0.139986f, -0.108620f,
            -0.081533f, -0.056811f, -0.033506f, -0.011115f,
             0.011115f,  0.033506f,  0.056811f,  0.081533f,
             0.108620f,  0.139986f,  0.179174f,  0.235859f
        };
