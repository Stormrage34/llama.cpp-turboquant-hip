...
// ---------------------------------------------------------------------------
// Optional ROCTx profiling markers (dlopen/dlsym, lazy, no-op when absent)
// Used by rocprofv3 --selected-regions for per-split kernel isolation.
// ---------------------------------------------------------------------------
#include <dlfcn.h>

namespace roctx_detail {

// Resolve roctx symbols via RTLD_DEFAULT so that rocprofv3's
// LD_PRELOAD interception (librocprofiler-sdk-roctx.so) takes priority.
inline void (*push_fn())(const char *) {
    static auto * p = [] {
        union { void * ptr; void (*fn)(const char *); } u;
        u.ptr = dlsym(RTLD_DEFAULT, "roctxRangePushA");
        return u.fn;
    }();
    return p;
}
inline void (*pop_fn() )() {
    static auto * p = [] {
        union { void * ptr; void (*fn)(); } u;
        u.ptr = dlsym(RTLD_DEFAULT, "roctxRangePop");
        return u.fn;
    }();
    return p;
}

inline void range_push(const char * name) { auto fn = push_fn(); if (fn) fn(name); }
inline void range_pop()                   { auto fn = pop_fn();  if (fn) fn();       }

struct marker {
    const char * name;
    explicit marker(const char * n) : name(n) { range_push(name); }
    ~marker() { range_pop(); }
};
} // namespace roctx_detail
...
void ggml_backend_tensor_set_async(ggml_backend_t backend, struct ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (backend->iface.set_tensor_async == NULL) {
        // use backend-specific sync when available to avoid blocking all backends
...
static enum ggml_status ggml_backend_sched_compute_splits(ggml_backend_sched_t sched) {
    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        // ROCTx range marker: name each split by backend + first op for
        // per-operation-group visibility in rocprofv3 traces.
        bool has_n0 = (split->graph.n_nodes > 0);
        const char * be_name = ggml_backend_name(split_backend);
        const char * op_name = has_n0 ? ggml_op_name(split->graph.nodes[0]->op) : "?";
        char split_marker[128];
        snprintf(split_marker, sizeof(split_marker), "splt:%s:%s", be_name, op_name);
        roctx_detail::marker _roctx_mark(split_marker);
        GGML_UNUSED(_roctx_mark);
