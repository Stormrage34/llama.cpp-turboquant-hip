...
static __device__ vec_dot_q_cuda_t get_vec_dot_q_cuda(ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_IQ4_XS:  return vec_dot_iq4_xs_q8_1_swizzled;
        case GGML_TYPE_Q4_K:    return vec_dot_q4_K_q8_1_swizzled;   // Intra-block swizzle
#if defined(RDNA2_SWIZZLE_ALL_QUANTS)
        case GGML_TYPE_Q5_K:    return vec_dot_q5_K_q8_1_swizzled;
#endif
#else
...
static __global__ void mul_mat_vec_q(
    ...
    const int rows_per_cuda_block = calc_rows_per_block(ncols_dst, table_id, small_k, nwarps);
...
#pragma unroll
    ...
    // Q4_K intra-block: d_swizzle_meta_offset > 0 means swizzled (intra-block layout),
    // 0 means unswizzled (AoS). Set by host via fusion struct.
...
__shared__ float tmp_shared[nwarps-1 > 0 ? nwarps-1 : 1][ncols_dst][rows_per_cuda_block][warp_size+1];
__shared__ float tmp_shared_gate[(has_fusion && (nwarps-1 > 0)) ? nwarps-1 : 1][ncols_dst][rows_per_cuda_block][warp_size+1];
    if constexpr (!has_fusion) {
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if (has_ids && ncols_dst > 1) {
        ...
        mul_mat_vec_q_moe_launch<type>(
            ...
            stride_row_x, stride_col_y, stride_col_dst,
            stride_channel_x, stride_channel_y, stride_channel_dst,
            ncols_dst, ids_stride, nchannels_x, warp_size, nchannels_dst, stream);
        return;
    }
...
void ggml_cuda_mul_mat_vec_q(
...
#if defined(RDNA2_CACHE_SWIZZLE)
    // Q4_K intra-block swizzle: check tensor flag (set at load time)
    if (src0->type == GGML_TYPE_Q4_K && (src0->flags & GGML_TENSOR_FLAG_SWIZZLED)) {
        fusion_local.swizzle_meta_offset = 1;
    }
#endif
...
#if defined(RDNA2_CACHE_SWIZZLE)
    // Set SoA meta section offset for swizzled types (via fusion struct, compatible with CUDA graphs)
    // FIX (ERR-HIP-IQ4XS-VIEW-DRIFT-026): Always reset to zero first, then compute view-aware
    // parent coordinate mapping. Prevents stale offset from previous kernel launch leaking into
    // subsequent launches of different tensor types.
    fusion_local.swizzle_meta_offset = 0;
    ...
    if (src0->type == GGML_TYPE_IQ4_XS
        ...
        if (src0->type == GGML_TYPE_Q4_K) {
            // Q4_K intra-block swizzle: check tensor flag (set at load time)
            if (src0->flags & GGML_TENSOR_FLAG_SWIZZLED) {
                fusion_local.swizzle_meta_offset = 1;
            }
        } else if (src0->view_src != nullptr) {
            // Compute metadata offset relative to the absolute root parent allocation boundaries.
            // The root parent holds the full SoA buffer; any view (even chained views) is a sub-region.
            const ggml_tensor * p = src0;
            while (p->view_src != nullptr) {
                p = (ggml_tensor *)p->view_src;
            }

            const int64_t parent_nblocks = ggml_nbytes(p) / ggml_type_size(p->type);
            size_t parent_meta_offset    = parent_nblocks * 128;

            // Physical address displacement of the view slice within the parent buffer
            size_t view_data_offset      = (uint8_t*)src0->data - (uint8_t*)p->data;

            // Logical row shift context: how many blocks of src0 type fit before the view
            size_t R = view_data_offset / ggml_row_size(src0->type, src0->ne[0]);

            // Metadata stride per block matches parent tensor geometry
            int64_t parent_blocks_per_row = (p->ne[0] + QK_K - 1) / QK_K;
            size_t meta_stride_bytes      = parent_blocks_per_row * IQ4_XS_META_SIZE_HOST;
